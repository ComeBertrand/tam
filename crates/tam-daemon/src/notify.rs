...
pub struct NotifyConfig {
    ...
    /// Placeholders: `{task}`, `{id}` (alias for task), `{state}`, `{old_state}`.
    pub command: String,
...
mod tests {
    ...
    fn fire_matching_state() {
        let config = NotifyConfig {
            command: "echo {task} {state} {old_state}".into(),
    ...
    #[test]
    fn id_alias_for_task() {
        let config = NotifyConfig {
            command: "echo {id}".into(),
            on_states: vec![AgentState::Input],
        };
        // {id} should work as alias for {task}
        fire_if_matching(&config, "test", AgentState::Working, AgentState::Input);
    }
}
